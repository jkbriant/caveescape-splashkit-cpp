...
sprite get_random_coin() {
  sprite result;
  result = create_sprite(bitmap_named("Coin"));
  sprite_set_x(result, screen_width() + rnd(1200));
  sprite_set_y(result, screen_height()/2 - sprite_height(result)/2 - 100 + rnd(200));
...
game_data set_up_game() {
  ...
  for (int i = 0; i < NUM_COINS; i++) {
    result.coins[i] = get_random_coin();
  }
...
void reset_coins(sprite &coins) {
  free_sprite(coins);
  coins = get_random_coin();
}

void update_coins(sprite &coins, player_data &player) {
  update_sprite(coins);

  if (sprite_x(coins) < (0 - sprite_width(coins))) {
    reset_coins(coins);
  }

  if (sprite_collision(coins, player.sprite_data)) {
    reset_coins(coins);
    ++player.score;
  }
...
void check_for_obstacle_overlap(game_data &game) {
  ...
  for (int i = 0; i < NUM_COINS; ++i) {
    for (int b = 0; b < NUM_POLES; ++b) {
      if (sprite_collision(game.coins[i], game.poles[b].up_pole)) {
        reset_coins(game.coins[i]);
        game.coins[i] = get_random_coin();
      }
    }
  }
}
...
void update_game(game_data &game) {
  if (!game.player.is_dead) {
    ...
    for (int i = 0; i < NUM_COINS; ++i) {
      update_coins(game.coins[i], game.player);
    }
